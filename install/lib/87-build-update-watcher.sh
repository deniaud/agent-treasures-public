#!/usr/bin/env bash
# Install build-update-{check,apply}.sh into ~/.claude/scripts/.
...
for f in build-update-check.sh build-update-apply.sh; do
  [[ -f "$SRC/$f" ]] || { echo "[!] missing source: $SRC/$f"; exit 1; }
done

[[ ${DRY_RUN:-0} -eq 1 ]] && {
  echo "[dry] install build-update-check.sh + build-update-apply.sh → $DST/"
  exit 0
}
