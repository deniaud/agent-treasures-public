...
step_5_apply_rewriter() {
  banner "5/6  cc-prompt-rewriter: хуки (bundle — в общем репаке шага 6)"
  ...
  # The bundle patch is applied for ALL tools in ONE repack at step 6
  # (cc-bundle-restack.mjs) to avoid the cumulative node-lief .bun bloat that
  # 3 stacked repacks (tweakcc → rewriter → quote) would cause. Here we only
  # install rewrite-mode's fallback hook + assets — no per-tool repack.
  local tmp_out; tmp_out=$(mktemp)
  cc-prompt-rewriter hooks >"$tmp_out" 2>&1
  local rc=$?
  cat "$tmp_out" >>"$LOG_FILE"
  if (( rc == 0 )); then
    mark_ok "apply-rewriter" "hooks installed (bundle via combined restack)"
    echo "  ✓ cc-prompt-rewriter hooks installed" >&2
  else
    local err; err=$(tail -3 "$tmp_out" | head -1)
    mark_fail "apply-rewriter" "cc-prompt-rewriter hooks rc=$rc: $err"
    echo "  ✗ cc-prompt-rewriter hooks fail (rc=$rc) — см. лог" >&2
  fi
  rm -f "$tmp_out"
  return 0
}
...
step_6_apply_ccquote() {
  banner "6/6  общий репак: cc-quote + cc-prompt-rewriter (один writeContent)"
  ...
  # Best-effort global cc-quote (for `cc-quote status`); not used for patching.
  if ! command -v cc-quote >/dev/null 2>&1; then
    install_global_from_local "cc-quote" "$CC_QUOTE_CLONE" || true
    hash -r 2>/dev/null || true
  fi

  # Single-repack bundle patch for BOTH tools (the cc-stack model): read the
  # tweakcc-patched bundle once, apply cc-quote's citation patches + cc-prompt-
  # rewriter's rewrite-mode patches in order, writeContent ONCE. This bounds the
  # binary to 2 repacks total (tweakcc + this) instead of 3, keeping it under
  # the fork's vaddr-gap bloat guard.
  local restack="$HOME/.claude/scripts/cc-bundle-restack.mjs"
  if [[ ! -f "$restack" ]]; then
    mark_fail "apply-ccquote" "combined restack script missing: $restack"
    echo "  ✗ cc-bundle-restack.mjs отсутствует" >&2
    return 0
  fi

  local tmp_out; tmp_out=$(mktemp)
  TWEAKCC_FORK="$HOME/dev/tweakcc-fixed" \
    CC_REWRITER_DIR="$CC_REWRITER_CLONE" \
    CC_QUOTE_DIR="$CC_QUOTE_CLONE" \
    node "$restack" >"$tmp_out" 2>&1
  local rc=$?
  cat "$tmp_out" >>"$LOG_FILE"
  if grep -qE 'anchor miss' "$tmp_out"; then
    local failed; failed=$(grep -oE '✗ [a-zA-Z0-9_-]+:[a-zA-Z0-9_-]+' "$tmp_out" | sed 's/^✗ //' | paste -sd, -)
    mark_fail "apply-ccquote" "anchor-mismatch ($failed) — обнови tool repo под текущую CC"
    echo "  ⚠ combined restack: anchor mismatch ($failed)" >&2
  elif (( rc == 0 )); then
    mark_ok "apply-ccquote" "combined restack applied (citations + rewrite, 1 repack)"
    echo "  ✓ общий репак применён (cc-quote + cc-prompt-rewriter)" >&2
  else
    local err; err=$(tail -3 "$tmp_out" | head -1)
    mark_fail "apply-ccquote" "combined restack rc=$rc: $err"
    echo "  ✗ общий репак упал (rc=$rc) — см. лог" >&2
  fi
  rm -f "$tmp_out"
  return 0
}
