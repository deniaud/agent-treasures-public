...
CCQ_NPM_VER="$(jq -r .version "$HOME/dev/cc-quote/package.json" 2>/dev/null || echo unknown)"
...
cc_quote_npm_version   = $CCQ_NPM_VER
...
SENSITIVE_FOR_PUBLIC=(
  ...
  "claude/.claude/mcp/openlove.json"
  "host/.config/pip"
...
declare -A SCRUB_LINES=(
  ["host/.zshrc"]='openlove|claude-ol'
  ["docs/aliases.md"]='openlove|OpenLove|OL endpoint|claude-ol'
)
